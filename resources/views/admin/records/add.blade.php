...
<script src="https://cdn.ckeditor.com/4.12.1/full/ckeditor.js"></script>
                <div class="content container-fluid">
					...
					<div class="row">
						<div class="col-sm-12">
							<div class="card">
								...
								<!-- Add Blog -->
                                <form method='post'  action="{{ route('records.create') }}"  enctype="multipart/form-data">
                    @csrf
									...
									<div class="service-fields mb-3">
										...
										<div class="form-group col-md-6">
                                                ...
                                                <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                                    <input class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" id="input-name" type="text" placeholder="{{ __('  Title') }}"  value="{{ old('name') }}"  aria-required="true"/>
                                                    @if ($errors->has('name'))
                                            ...
                                            No, it’s free.
                                                ...
                                                <input class="form-control{{ $errors->has('price') ? ' is-invalid' : '' }}" name="price" id="input-price" type="text" placeholder="{{ __(' Price') }}"  value="{{ old('price') }}"  aria-required="true"/>
                                                @if ($errors->has('price'))
                    ...
                    <select  class="custom-select {{ $errors->has('category') ? ' is-invalid' : '' }}" name='category' id="input-category"   >
                            ...
                            <option value='people' {{ ('people'==old('category'))?'selected':''}}> people </option>  
                            <option value='tech' {{ ('tech'==old('category'))?'selected':''}}> tech</option>
                            <option value='entertainment' {{ ('entertainment'==old('category'))?'selected':''}}> entertainment</option>
